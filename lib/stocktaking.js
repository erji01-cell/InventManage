...
// 基準日時点のシステム在庫（帳簿値）を全資産分まとめて算出する。
// opening_stock + (基準日までの入庫 − 出庫)。年度クローズ済み期間は opening_stock に
// 反映済みのため除外。棚卸し開始時のスナップショットと、確定時のライブ再計算の
// 両方でこの同一ロジックを使い、計算のズレを防ぐ。
export function buildSystemQtyMap(assets, movements, basisDate) {
  ...
  assets.forEach((a) => {
    ...
    result.set(key, Number(a.openingStock || 0)
...
export async function createStocktaking({ staffId, memo, basisDate, assets, movements }, session) {
  const systemQtyMap = buildSystemQtyMap(assets, movements, basisDate);
  ...
  const items = assets.map((a) => {
    const key = String(a.id);
    const systemQty = systemQtyMap.get(key) || 0;
    return {
      count_id: count.id,
