...
import { normalizeOrderRequest } from '../utils/orders.js';
...
async function performSessionRefresh(session) {
  ...
  try {
    ...
    // ネットワーク断（スリープ復帰直後等）。セッションは破棄せず、次回の
    // フォーカス更新・定期処理でそのまま再試行できるようにする。
    throw new Error('通信エラーによりログイン状態を更新できませんでした。接続を確認してください。');
  }
  const payload = await response.json().catch(() => null);

  if (!response.ok) {
    // トークンが本当に無効（400/401/403）な場合のみ「期限切れ」としてセッション破棄。
    // 5xx や 429 などの一時的なエラーでセッションを捨てると、有効なログイン情報まで
    // 失って不要な再ログインを強いることになるため、破棄せず再試行に回す。
    ...
    throw new Error('サーバーが混み合っているためログイン状態を更新できませんでした。しばらくして再試行してください。');
  }
...
async function fetchTable(tableName, orderBy = 'id.asc', session = null, extraFilter = '') {
  const rows = [];

  for (let offset = 0; ; offset += PAGE_SIZE) {
    const page = await supabaseRequest(
      `${tableName}?select=*${extraFilter}&order=${orderBy}&limit=${PAGE_SIZE}&offset=${offset}`,
      {},
      session
    );
    rows.push(...page);
...
// 指定した会計年度（7/1〜翌6/30）の入出庫を生の行のまま取得する。
// 過去年度の閲覧時に必要な年だけ遅延読み込みするために使う。
export async function fetchMovementsForFiscalYear(startYear, session) {
  return fetchTable(
    'invent_stock_movements',
    'movement_date.desc',
    session,
    `&movement_date=gte.${startYear}-07-01&movement_date=lte.${startYear + 1}-06-30`
  );
}

export async function loadInventoryData(session) {
  // 直近の年度クローズ日を先に取得し、入出庫は「クローズ日より後」だけ読み込む。
  // クローズ日以前の分は opening_stock / スナップショットに反映済みで
  // 現在年度の計算には使われないため、起動を軽くできる（年々増えない）。
  // 過去年度の閲覧時は fetchMovementsForFiscalYear で必要な年だけ追加読み込みする。
  let latestClosedAt = null;
  try {
    const closedRows = await supabaseRequest(
      'invent_child_assets?select=fiscal_year_closed_at&fiscal_year_closed_at=not.is.null&order=fiscal_year_closed_at.desc&limit=1',
      {},
      session
    );
    latestClosedAt = closedRows[0]?.fiscal_year_closed_at || null;
  } catch {
    latestClosedAt = null; // 取得失敗時は従来どおり全件読み込みにフォールバック
  }
  const movementFilter = latestClosedAt ? `&movement_date=gt.${latestClosedAt}` : '';

  const [suppliers, staff, parents, childAssets, movements, categories, orderRequests] = await Promise.all([
    ...
    fetchTable('invent_stock_movements', 'movement_date.desc', session, movementFilter),
    fetchTable('invent_categories', 'display_order.asc', session),
    // SQL適用前の環境でも既存機能は起動できるよう、発注テーブルだけ任意扱いにする。
    fetchTable('invent_order_requests', 'requested_at.desc', session).catch(() => []),
  ]);
  ...
  return {
    ...
    orderRequests: orderRequests.map(normalizeOrderRequest),
