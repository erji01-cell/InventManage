...
// - Auto-backup: 起動時＋変更を3分ごとに確認して保存（変更なしスキップ・1日1ファイル）
...
const OPTIONAL_BACKUP_TABLES = new Set(['invent_fiscal_snapshots', 'invent_order_requests']);
...
// fetchのタイムアウト。応答が返らないままハングすると _backupInFlight が
// 占有され続け、以後のバックアップがすべて素通りになるのを防ぐ
...
async function fetchAllRows(table, session) {
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const res = await fetch(
      ...
      { headers: await authHeaders(session), signal: fetchTimeout() }
    );
    if (!res.ok) {
      ...
      if (backups.length > 0) {
        ...
        if (isFresh) {
          ...
          if (JSON.stringify(latestPayload?.tables) === JSON.stringify(payload.tables)) {
            ...
            const pruneResult = prune ? await pruneOldBackups(session, KEEP_COUNT) : null;
            return { skipped: true, fileName: latest.name, payload, pruneResult };
          }
        }
      }
    } catch {
