...
function makeFileName(date = new Date()) {
  ...
  return `backup_${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}_${pad(date.getHours())}-${pad(date.getMinutes())}-${pad(date.getSeconds())}.json`;
}
...
export async function listStorageBackups(session) {
  ...
  return (items || []).filter((it) => it.name && it.name.endsWith('.json'));
}
...
// Best-effort backup on shutdown using sendBeacon (no auth header support → falls back to fetch keepalive).
export function scheduleBeforeUnloadBackup(session) {
  const handler = () => {
    if (!isAutoBackupEnabled()) return;
    // Quick check: don't spam — only if >1 hour since last backup
    const last = getLastBackupTime();
    if (last && (Date.now() - last) / 1000 / 60 < 60) return;

    // We can't easily build a full backup synchronously, so fire a flag that the next
    // startup picks up. Also try a fetch keepalive ping to record intent.
    try {
      localStorage.setItem('invent_manage_pending_backup', '1');
    } catch {
      /* ignore */
    }
  };
...
export function consumePendingBackup() {
  const v = localStorage.getItem('invent_manage_pending_backup');
  localStorage.removeItem('invent_manage_pending_backup');
  return v === '1';
}
