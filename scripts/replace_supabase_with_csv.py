...
# invent_staff は「変更しない」運用方針のため、このスクリプトでは一切触らない
# （削除・投入の対象から除外）。担当者は現行DBの値を常に正とする。
# invent_categories も移行対象外で触らない。
...
# stock_movements は staff_code を参照するが、staff は消さないので FK 上も問題なし。
DELETE_ORDER = [
    "invent_stock_movements",
